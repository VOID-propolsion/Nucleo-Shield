...
u32_t W25Q_sector_to_page(u32_t sector_id);
 ...
 * @return W25Q_STATE enum
 */
W25Q_STATE W25Q_ReadSector(u8_t *buf, u32_t sectorNum, u32_t bytes_to_read_upto_sector_size, u32_t offset)
{
    if ((bytes_to_read_upto_sector_size > MEM_SECTOR_SIZE) || (bytes_to_read_upto_sector_size == 0))
    {
        bytes_to_read_upto_sector_size = MEM_SECTOR_SIZE;
    }

    if (offset >= MEM_SECTOR_SIZE)
    {
        return W25Q_PARAM_ERR;
    }

    uint32_t pageStart = W25Q_sector_to_page(sectorNum) + (offset / MEM_PAGE_SIZE);
    uint32_t pageShift = offset % MEM_PAGE_SIZE;
    int32_t bytes_to_read = 0;

    if ((offset + bytes_to_read_upto_sector_size) > MEM_SECTOR_SIZE)
    {
        bytes_to_read = static_cast<int32_t>(MEM_SECTOR_SIZE - offset);
    }
    else
    {
        bytes_to_read = bytes_to_read_upto_sector_size;
    }

    while (bytes_to_read > 0)
    {
        W25Q_STATE status = W25Q_Read_Page_SingleSPI(buf, pageStart, bytes_to_read, pageShift);
        if (status != W25Q_OK)
        {
            return status;
        }
        bytes_to_read -= static_cast<int32_t>(MEM_PAGE_SIZE - pageShift);
        buf += MEM_PAGE_SIZE - pageShift;
        pageStart++;
        pageShift = 0;
    }
 ...
 * @return W25Q_STATE enum
 */
W25Q_STATE W25Q_Read_Page_DualIO(u8_t *buf, u16_t data_len, u32_t rawAddr)
{
    ...
    com.Instruction = W25Q_FAST_READ_DUAL_IO; // Command
    ...
    w25q_delay(1); // Give a little time to read
 ...
 * @return W25Q_STATE enum
 */
W25Q_STATE W25Q_Read_Page_SingleSPI(u8_t *buf, u32_t pageNum, u32_t bytes_to_read_upto_pagesize, u32_t offset)
{

    // if the page number is out of range than just set bytes_to_write_upto_pagesize to pagesize
    if ((bytes_to_read_upto_pagesize > MEM_PAGE_SIZE) || bytes_to_read_upto_pagesize == 0)
    {
        bytes_to_read_upto_pagesize = MEM_PAGE_SIZE;
    }

    if ((offset + bytes_to_read_upto_pagesize) > MEM_PAGE_SIZE)
    {
        bytes_to_read_upto_pagesize = MEM_PAGE_SIZE - offset;
    }

    pageNum = pageNum * MEM_PAGE_SIZE + offset;
    ...
    com.Address = pageNum;
    com.Instruction = W25Q_READ_DATA;
    ...
    com.DummyCycles = 0;
    com.DataMode = QSPI_DATA_1_LINE;
    com.NbData = bytes_to_read_upto_pagesize;
 ...
 * @return W25Q_STATE enum
 */
W25Q_STATE W25Q_ProgramSector(u8_t *buf, u32_t sector_num, u32_t bytes_to_write_upto_sector_size, u32_t offset)
{
    // if the bytes to write of range (0..4096), than set it to 4096
    if ((bytes_to_write_upto_sector_size > MEM_SECTOR_SIZE) || bytes_to_write_upto_sector_size == 0)
    {
        bytes_to_write_upto_sector_size = MEM_SECTOR_SIZE;
    }
    ...
    uint32_t pageStart = W25Q_sector_to_page(sector_num) + (offset / MEM_PAGE_SIZE);
    uint32_t pageShift = offset % MEM_PAGE_SIZE;
    int32_t bytes_to_write = 0;

    // Normalize bytes to write if they are out of sector size
    if ((offset + bytes_to_write_upto_sector_size) > MEM_SECTOR_SIZE)
    {
        bytes_to_write = static_cast<uint32_t>(MEM_SECTOR_SIZE - offset);
    }
    else
    {
        bytes_to_write = bytes_to_write_upto_sector_size;
    }

    while (bytes_to_write > 0)
    {
        W25Q_STATE status = W25Q_ProgramPage(buf, pageStart, bytes_to_write, pageShift);
        if (status != W25Q_OK)
        {
            return status;
        }
        bytes_to_write -= static_cast<int32_t>(MEM_PAGE_SIZE - pageShift);
        buf += MEM_PAGE_SIZE - pageShift;
        pageStart++;
        pageShift = 0;
    }
 ...
 * @return W25Q_STATE enum
 */
W25Q_STATE W25Q_ProgramPage(u8_t *buf, u32_t pageNum, u32_t bytes_to_write_upto_pagesize, u32_t offset)
{

    // if the page number is out of range than just set bytes_to_write_upto_pagesize to 0
    if (((bytes_to_write_upto_pagesize + offset) > MEM_PAGE_SIZE) || bytes_to_write_upto_pagesize == 0)
    {
        bytes_to_write_upto_pagesize = MEM_PAGE_SIZE - offset;
    }

    if ((offset + bytes_to_write_upto_pagesize) > MEM_PAGE_SIZE)
    {
        bytes_to_write_upto_pagesize = MEM_PAGE_SIZE - offset;
    }
    ...
    com.DummyCycles = 0;
    com.DataMode = QSPI_DATA_1_LINE;
    com.NbData = bytes_to_write_upto_pagesize;
...
{
    if (SectAddr >= MEM_SECTOR_SIZE)
        return W25Q_PARAM_ERR;

    while (W25Q_IsBusy() == W25Q_BUSY)
        w25q_delay(1);

    u32_t rawAddr = SectAddr * MEM_SECTOR_SIZE;
...
W25Q_STATE W25Q_EraseBlock(u32_t BlockAddr, u8_t size)
{
    ...
    u32_t rawAddr = BlockAddr * MEM_SECTOR_SIZE * 16;
    if (size == 32)
        rawAddr /= 2;
...
u32_t W25Q_sector_to_page(u32_t sector_id)
{
    return (sector_id * MEM_SECTOR_SIZE) / MEM_PAGE_SIZE;
}
